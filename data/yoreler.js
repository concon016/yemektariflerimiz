// yemektariflerimiz — bölge (yöre) verisi
// CommonJS: scripts/build.js tarafından okunur.

module.exports = [
  {
    slug: "ege",
    ad: "Ege",
    bolumNo: "Bölüm I",
    emoji: "🫒",
    renk: "#C9992F",
    aciklama:
      "Zeytinyağı, taze ot ve deniz ürünlerinin hâkim olduğu Ege mutfağı, hafif ama derin lezzetli bir sofra geleneğidir. İzmir'den Bodrum'a uzanan kıyı şeridinde, sebzeler çoğu zaman etsiz ve zeytinyağlı pişirilir; sokak lezzetleri ise kendine has bir kimlik taşır.",
    metaAciklama:
      "Zeytinyağlılardan İzmir sokak lezzetlerine: Ege mutfağının en bilinen tariflerini hikayeleriyle, adım adım anlatımla keşfedin.",
  },
  {
    slug: "karadeniz",
    ad: "Karadeniz",
    bolumNo: "Bölüm II",
    emoji: "🌊",
    renk: "#556B3D",
    aciklama:
      "Mısır, hamsi ve yeşilin buluştuğu Karadeniz sofraları, sade malzemelerle güçlü lezzetler çıkarmasıyla tanınır.",
    metaAciklama: "Karadeniz mutfağının yöresel tarifleri yakında bu bölümde.",
  },
  {
    slug: "akdeniz",
    ad: "Akdeniz",
    bolumNo: "Bölüm III",
    emoji: "🌶️",
    renk: "#A6402C",
    aciklama:
      "Narenciye, zeytinyağı ve acı baharatların dengelendiği Akdeniz mutfağı, güneşin sofraya yansıyan hâlidir.",
    metaAciklama: "Akdeniz mutfağının yöresel tarifleri yakında bu bölümde.",
  },
  {
    slug: "guneydogu-anadolu",
    ad: "Güneydoğu Anadolu",
    bolumNo: "Bölüm IV",
    emoji: "🍢",
    renk: "#7d2f1f",
    aciklama:
      "Kebap kültürünün ve baharatın zirve yaptığı Güneydoğu mutfağı, közün ve emeğin buluştuğu bir gelenektir.",
    metaAciklama: "Güneydoğu Anadolu mutfağının yöresel tarifleri yakında bu bölümde.",
  },
  {
    slug: "ic-anadolu",
    ad: "İç Anadolu",
    bolumNo: "Bölüm V",
    emoji: "🥟",
    renk: "#8a6d3b",
    aciklama:
      "Hamur işlerinin ustalıkla işlendiği İç Anadolu mutfağı, kışa hazırlık kültürüyle de bilinir.",
    metaAciklama: "İç Anadolu mutfağının yöresel tarifleri yakında bu bölümde.",
  },
  {
    slug: "dogu-anadolu",
    ad: "Doğu Anadolu",
    bolumNo: "Bölüm VI",
    emoji: "🏔️",
    renk: "#3d4f2b",
    aciklama:
      "Zorlu kış şartlarının şekillendirdiği Doğu Anadolu mutfağı, et ve süt ürünlerini ustaca değerlendirir.",
    metaAciklama: "Doğu Anadolu mutfağının yöresel tarifleri yakında bu bölümde.",
  },
  {
    slug: "marmara",
    ad: "Marmara",
    bolumNo: "Bölüm VII",
    emoji: "🥐",
    renk: "#4b5f7a",
    aciklama:
      "Osmanlı saray mutfağının izlerini taşıyan Marmara mutfağı, İstanbul'un kozmopolit lezzet mirasını yansıtır.",
    metaAciklama: "Marmara mutfağının yöresel tarifleri yakında bu bölümde.",
  },
];
